 ...
 * Each pixel row is an array of uint8s (which typically are unsigned chars).
...
// extern uint8* image_buffer = NULL;   /* Points to large array of R,G,B-order data */
// extern int image_height = 100;        /* Number of rows in image */
// extern int image_width = 100;         /* Number of columns in image */
...
GLOBAL(void)
write_JPEG_file(uint8* image_buffer, int image_height, int image_width,
    int input_components, char *filename)
{
  ...
  uint8* row_pointer[1];      /* pointer to uint8 row[s] */
   ...
   */
  row_stride = image_width * input_components; /* uint8s per row in image_buffer */
 ...
 * scanline-high uint8 array as a work buffer, and we will let the JPEG
...
GLOBAL(int)
read_JPEG_file(uint8*& img, int& height, int& width,
    int& components, char *filename)
{
   ...
   */
  /* uint8s per row in output buffer */
  row_stride = cinfo.output_width * cinfo.output_components;
  height = cinfo.output_height;
  width = cinfo.output_width;
  components = cinfo.output_components;
  printf("Reading image: height=%d, width=%d, components=%d\n",
    height, width, components);
  img = new uint8[height*width*components];
  
  /* Make a one-row-high sample array that will go away when done with image */
  buffer = (uint8*)(*cinfo.mem->alloc_sarray)
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    for (int j=0; j<width; j++) {
        for (int c=0; c<components; c++) {
            img[(cinfo.output_scanline-1)*row_stride+j*components+c] = 
                buffer[j*components+c];
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    char* sfilename = argv[2];
    printf("filename: %s\n", filename);
    uint8* img = NULL;
    int height, width, channel;
    read_JPEG_file(img, height, width, channel, filename);
    printf("Finished reading file.\n");

    write_JPEG_file(img, height, width, channel, sfilename);
